...
def sort():
    ...
    for image in imageDataDownRGB.data:
        ...
        if(abs(float(xDownRGB)-float(image[2])) > 2) or (abs(float(yDownRGB)-float(image[3])) > 2):
            imagesDownRGB.append(usefulImages(r"data\ASPIRE_forDistro\1 Downlooking\RGB\image_{}.png".format(image[1]),image[2],image[3]))
            xDownRGB = image[2]
            yDownRGB = image[3]
    ...
    with open(r"code\usefulIamges\downlookingRGB.csv", mode="w", newline="") as file:
        file.truncate(0)
    ...
    for image in imageDataSideRGB.data:
        ...
        if(abs(float(xSideRGB)-float(image[2])) > 2) or (abs(float(ySideRGB)-float(image[3])) > 2):
            imagesSideRGB.append(usefulImages(r"data\ASPIRE_forDistro\2 Sidelooking\RGB\image_{}.png".format(image[1]),image[2],image[3]))
            xSideRGB = image[2]
            ySideRGB = image[3]
    ...
    with open(r"code\usefulIamges\sidelookingRGB.csv", mode="w", newline="") as file:
        file.truncate(0)
        writer = csv.writer(file)
    ...
    for image in imageDataDownLWIR.data:
        ...
        if(abs(float(xDownLWIR)-float(image[2])) > 2) or (abs(float(yDownLWIR)-float(image[3])) > 2):
            ...
            imagesDownLWIR.append(usefulImages(r"data\ASPIRE_forDistro\1 Downlooking\LWIR\image_{}.png".format(frame),image[2],image[3]))
            xDownLWIR = image[2]
            yDownLWIR = image[3]
    ...
    with open(r"code\usefulIamges\downlookingLWIR.csv", mode="w", newline="") as file:
        file.truncate(0)
        writer = csv.writer(file)
    ...
    for image in imageDataSideLWIR.data:
        ...
        if(abs(float(xSideLWIR)-float(image[2])) > 2) or (abs(float(ySideRGB)-float(image[3])) > 2):
            frame = str(image[1]).zfill(8)
            imagesSideLWIR.append(usefulImages(r"data\ASPIRE_forDistro\2 Sidelooking\LWIR\image_{}.png".format(frame),image[2],image[3]))
            xSideLWIR = image[2]
            ySideLWIR = image[3]
    ...
    with open(r"code\usefulIamges\sidelookingLWIR.csv", mode="w", newline="") as file:
        file.truncate(0)
        writer = csv.writer(file)
